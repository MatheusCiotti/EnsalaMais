...
class _RoomsManagementScreenState extends State<RoomsManagementScreen> {
  ...
  // Antes: List<Map<String, dynamic>> rooms = [];
  // Agora:
  List<Room> rooms = [];
  ...
  Future<void> _loadRooms() async {
    try {
      // Supondo que BlocksService.getRooms() retorne uma List<Map<String, dynamic>>
      final roomsData = await BlocksService.getRooms(widget.block.id);
      if (mounted) {
        setState(() {
          // A conversão de Map para Room é feita aqui
    ...
    showDialog(
      ...
      builder: (context) => StatefulBuilder(
        builder: (context, setDialogState) => AlertDialog(
          ...
          title: const Text('Adicionar Nova Sala', style: TextStyle(color: Colors.white)),
            ...
            child: Column(
              ...
              children: [
                ...
                TextField(
                  ...
                  style: const TextStyle(color: Colors.white),
                  maxLines: 2,
                  decoration: InputDecoration(labelText: 'Descrição (opcional)', labelStyle: const TextStyle(color: Colors.white), hintStyle: TextStyle(color: Colors.white.withOpacity(0.5)), enabledBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(30), borderSide: BorderSide(color: Colors.white.withOpacity(0.3))), focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(30), borderSide: const BorderSide(color: Colors.orange))),
                ),
                const SizedBox(height: 24),
                const Text('Equipamentos', style: TextStyle(color: Colors.white, fontSize: 16, fontWeight: FontWeight.bold)),
                CheckboxListTile(title: const Text('Ar Condicionado', style: TextStyle(color: Colors.white)), value: hasAirConditioning, onChanged: (value) { setDialogState(() { hasAirConditioning = value ?? false; }); }, checkColor: Colors.black, activeColor: Colors.orange),
                CheckboxListTile(title: const Text('Projetor', style: TextStyle(color: Colors.white)), value: hasProjector, onChanged: (value) { setDialogState(() { hasProjector = value ?? false; }); }, checkColor: Colors.black, activeColor: Colors.orange),
                CheckboxListTile(title: const Text('TV', style: TextStyle(color: Colors.white)), value: hasTV, onChanged: (value) { setDialogState(() { hasTV = value ?? false; }); }, checkColor: Colors.black, activeColor: Colors.orange),
              ],
            ),
          ),
          actions: [
              ...
              onPressed: () async {
                if (nomeController.text.isNotEmpty) {
                  try {
                    ...
                    if (mounted) {
                      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Erro ao criar sala: $e')));
                    }
                  }
                }
              },
  ...
  void _showEditRoomDialog(Room room, int index) {
    ...
    showDialog(
      ...
      builder: (context) => StatefulBuilder(
        builder: (context, setDialogState) => AlertDialog(
          backgroundColor: Colors.grey[900],
          title: const Text('Editar Sala', style: TextStyle(color: Colors.white)),
          content: SingleChildScrollView(
            child: Column(
              mainAxisSize: MainAxisSize.min,
              children: [
                TextField(controller: nomeController, style: const TextStyle(color: Colors.white), decoration: InputDecoration(labelText: 'Nome da Sala', labelStyle: const TextStyle(color: Colors.white), hintStyle: TextStyle(color: Colors.white.withOpacity(0.5)), enabledBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(30), borderSide: BorderSide(color: Colors.white.withOpacity(0.3))), focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(30), borderSide: const BorderSide(color: Colors.orange)))),
                const SizedBox(height: 16),
                          ...
                          return Card(
                            ...
                            child: Column(
                              ...
                              children: [
                                ListTile(
                                  title: Text(
                                    // Adicionada verificação de nulo aqui também
                                    room.nome ?? 'Sala sem nome', 
                                  ...
                                  subtitle: Column(
                                    ...
                                    children: [
                                      if (room.descricao != null)
                                        Text(room.descricao!, style: TextStyle(color: Colors.white.withOpacity(0.7))),
                                    ],
                                  ),
                                  trailing: Row(
                                    mainAxisSize: MainAxisSize.min,
                                    children: [
                                      IconButton(icon: const Icon(Icons.edit, color: Colors.orange), onPressed: () => _showEditRoomDialog(room, index)),
                                      IconButton(icon: const Icon(Icons.delete, color: Colors.red), onPressed: () => _showDeleteConfirmationDialog(room, index)),
                                    ],
                                  ),
                                ),
                                if (room.hasAirConditioning || room.hasProjector || room.hasTV)
                                  Padding(
                                    ...
                                    child: Wrap(
                                      spacing: 8,
                                      children: [
                                        if (room.hasAirConditioning) Chip(backgroundColor: Colors.blue.withOpacity(0.2), label: const Text('Ar Condicionado', style: TextStyle(color: Colors.white))),
                                        if (room.hasProjector) Chip(backgroundColor: Colors.green.withOpacity(0.2), label: const Text('Projetor', style: TextStyle(color: Colors.white))),
                                        if (room.hasTV) Chip(backgroundColor: Colors.purple.withOpacity(0.2), label: const Text('TV', style: TextStyle(color: Colors.white))),
                                      ],
                                    ),
                                  ),
                              ],
